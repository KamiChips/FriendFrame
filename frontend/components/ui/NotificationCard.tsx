...
const NotificationCard = ({
  ...
  switch (notificationType) {
    case NotificationType.LIKE:
      message = (
        <>
          <Text className="font-spartan-bold">{name}</Text> le dio like a tu
          post
        </>
      );
      icon = (
    ...
    case NotificationType.COMMENT:
      message = (
        <>
          <Text className="font-spartan-bold">{name}</Text> comentó: "{comment}"
        </>
      );
      icon = (
        <FontAwesome
    ...
    case NotificationType.POST:
      message = (
        <>
          <Text className="font-spartan-bold">{name}</Text> publicó en tu perfil
        </>
      );
      icon = (
        <FontAwesome
    ...
    case NotificationType.FOLLOW:
      message = (
        <>
          <Text className="font-spartan-bold">{name}</Text> comenzó a seguirte
        </>
      );
      icon = (
        <SimpleLineIcons
    ...
    case NotificationType.MENTION:
      message = (
        <>
          <Text className="font-spartan-bold">{name}</Text> te envió un mensaje:
          "{comment}"
        </>
      );
      icon = (
  ...
  return (
    ...
    >
      ...
      {unread && (
        <View className="justify-end w-3 h-3 ml-3 bg-primary-light rounded-full dark:bg-tertiary-dark"></View>
      )}
    </TouchableOpacity>
  );
};
