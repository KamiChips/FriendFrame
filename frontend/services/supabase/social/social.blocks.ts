...
import { fetchBlockStatus } from "./social.queries";
import { normalizePagination, parseError } from "./social.helper";
...
export async function unblockUser(targetUserId: string): Promise<SocialResult> {
  try {
    assertUUID(targetUserId, "ID de usuario");
    const currentUserId = await getAuthUser();
...
export async function isBlocked(
  ...
  SocialResult<{ i_blocked_them: boolean; they_blocked_me: boolean }>
> {
  try {
    assertUUID(targetUserId, "ID de usuario");
    const currentUserId = await getAuthUser();
...
export async function getBlockedUsers(
  ...
  SocialResult<
    (Pick<SocialUser, "user_id" | "full_name" | "username" | "profile_pic"> & {
      blocked_at: string;
    })[]
  >
> {
