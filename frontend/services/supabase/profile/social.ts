...
import { PaginationParams, ProfileResult, ProfileStats, SearchResult, UserProfile } from "./types"
...
export async function getFollowers(
  ...
  try {
    ...
    const { data, error } = await supabase
      .rpc('get_followers_with_relationship', {
...
export async function getFollowing(
  ...
  try {
    ...
    const { data, error } = await supabase
      .rpc('get_following_with_relationship', {
        target_user_id:  userId,
        current_user_id: currentUserId,
        p_limit:         limit,
        p_offset:        offset,
